...
const { FetchUserLogs } = require("../monkie/reponses");
  ...
  const logsCmd = async (msg, args, thread) => {
    ...
    const threadLines = await Promise.all(
      userThreads.map(async (thread) => {
        ...
        return `**\`#${thread.thread_number}\`** [View](${formattedLogUrl}) \`${formattedDate}\``;
      })
    );

    let message = isPaginated
      ? `**Log(s) for user: <@${userId}>**\n**Page:** \`${page}/${maxPage}\`\n**Showing logs:** \`${
          start + 1
        }-${end}/${totalUserThreads}\``
      : `**Log(s) for user: <@${userId}>\n**`;

    message += `\n${threadLines.join("\n")}`;

    if (isPaginated) {
      message += `\n\n**_To view more logs, add a page number to the end of the command (_**_Example \`${config.prefix}logs 2\`_**_)_**`;
    }
    ...
    chunks.forEach((lines) => {
      root = root.then(() =>
        msg.channel.createMessage(FetchUserLogs(lines.join("\n")))
      );
    });
  };
  ...
  const logCmd = async (msg, args, _thread) => {
    ...
    if (logUrl) {
      msg.channel.createMessage(
        `Open the following link to view the log for thread #${
    ...
    if (logFile) {
      msg.channel.createMessage(
        `Download the following file to view the log for thread #${thread.thread_number}:`,
        logFile
      );
      return;
    }

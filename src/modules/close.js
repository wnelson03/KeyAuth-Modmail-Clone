...
const { ClosedTicketLog } = require("../monkie/reponses");
...
module.exports = ({ bot, knex, config, commands }) => {
  async function sendCloseNotification(thread, body) {
    ...
    if (logUrl) {
      utils.postLog(ClosedTicketLog(body, logUrl));
      return;
    }
  ...
  async function applyScheduledCloses() {
    ...
    for (const thread of threadsToBeClosed) {
      ...
      await sendCloseNotification(
        thread,
        `Modmail thread \`#${thread.thread_number}\` with <@${thread.user_id}> was closed as scheduled by **${thread.scheduled_close_name}**`
      );
    }
  }
  ...
  commands.addGlobalCommand(
    ...
    async (msg, args) => {
      ...
      await sendCloseNotification(
        thread,
        `Modmail thread \`#${thread.thread_number}\` with <@${
          thread.user_id
        }> was closed by ${
          closedBy == "the user" ? `${closedBy}.` : `<@!${closedBy}>.`
        }`
      );
    },
    {
  ...
  bot.on("channelDelete", async (channel) => {
    ...
    await sendCloseNotification(
      thread,
      `Modmail thread \`#${thread.thread_number}\` with <@${thread.user_id}> was closed automatically because the channel was deleted`
    );
  });
};

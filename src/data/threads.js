...
function _addToThreadCreationQueue(fn) {
  threadCreationQueue = threadCreationQueue.then(fn).catch((err) => {
...
async function findById(id) {
  const thread = await knex("threads").where("id", id).first();
  ...
  return _addToThreadCreationQueue(async () => {
    ...
    if (config.requiredAccountAge && !ignoreRequirements) {
      if (user.createdAt > moment() - config.requiredAccountAge * HOURS) {
        if (config.accountAgeDeniedMessage) {
          const accountAgeDeniedMessage = utils.readMultilineConfigValue(
    ...
    if (config.requiredTimeOnServer && !ignoreRequirements) {
      ...
      const isAllowed =
        ...
        Array.from(userGuildData.values()).some(({ guild, member }) => {
          return (
            member.joinedAt < moment() - config.requiredTimeOnServer * MINUTES
          );
        });

      if (!isAllowed) {
        if (config.timeOnServerDeniedMessage) {
          const timeOnServerDeniedMessage = utils.readMultilineConfigValue(
    ...
    const isPaid =
      ...
      Array.from(userGuildData.values()).some(({ guild, member }) => {
        ...
        member.roles.forEach((role) => {
          if (PaidRoles.includes(role)) paidMember = true;
        });
        return paidMember;
        // member.roles.includes("824406803827392523") ||
        // member.roles.includes("824406788581490708") ||
        // member.roles.includes("949428085852356638")
      });
    ...
    let createdChannel;
    try {
      createdChannel = await utils
        .getInboxGuild()
        .createChannel(channelName, DISOCRD_CHANNEL_TYPES.GUILD_TEXT, {
          reason: "New Modmail thread",
          parentID: newThreadCategoryId,
        });
    } catch (err) {
      console.error(
        `Error creating modmail channel for ${user.username}#${user.discriminator}!`
      );
      throw err;
    }
    ...
    if (!quiet) {
      ...
      const staffMention = opts.mentionRole
        ? utils.mentionRolesToMention(
            utils.getValidMentionRoles(opts.mentionRole)
          )
        : utils.getInboxMention();

      if (staffMention.trim() !== "") {
        const allowedMentions = opts.mentionRole
          ? utils.mentionRolesToAllowedMentions(
              utils.getValidMentionRoles(opts.mentionRole)
            )
    ...
    infoHeaderItems.push(`**Created By:** \`${newThread.user_name}\``);

    // User id (and mention, if enabled)
    if (config.mentionUserInThreadHeader) {
      infoHeaderItems.push(`**Username:** <@!${user.id}>`);
    }
    ...
    for (const [guildId, guildData] of userGuildData.entries()) {
      ...
      if (config.rolesInThreadHeader && guildData.member.roles.length) {
        const roles = guildData.member.roles
          .map((roleId) => guildData.guild.roles.get(roleId))
          .filter(Boolean);
        headerItems.push(
    ...
    if (userLogCount > 0) {
      infoHeader += `\n**Prev Thread Count:** \`${userLogCount}\`\n\n**Staff Help:**\n\`${config.prefix}logs\` To see previous threads.\n\`${config.prefix}close\` Close this ticket.\n\`${config.prefix}block\` or \`${config.prefix}block <time>\` Block user from tickets.`;
    } else {
      infoHeader += `\n\n**Staff Help:**\n\`${config.prefix}close\` Close this ticket.\n\`${config.prefix}block\` or \`${config.prefix}block <time>\` Block user from tickets.`;
    }
    const reply = FreshTicketMessage(infoHeader);
    const { message: threadHeaderMessage } = await newThread.postSystemMessage(
      reply,
      {
        allowedMentions: config.mentionUserInThreadHeader
          ? { users: [user.id] }
          : undefined,
      }
    );
    ...
    //   if (availableUpdate) {
    //     await newThread.postNonLogMessage(
    //       `📣 New bot version available (${availableUpdate})`
    //     );
    //   }
    // }
...
async function createThreadInDB(data) {
  ...
  const latestThreadNumberRow = await knex("threads")
    ...
    .first();
  const latestThreadNumber = latestThreadNumberRow
    ? latestThreadNumberRow.thread_number
    : 0;
  const finalData = Object.assign({ created_at: now, is_legacy: 0 }, data, {
    id: threadId,
    thread_number: latestThreadNumber + 1,
  });
